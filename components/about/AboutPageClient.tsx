...
import { about, experience, skills, site } from "@/lib/data";
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:gap-16">
          <div className="space-y-6">
            ...
            {about.body.map((paragraph, index) => (
              <FadeIn key={paragraph.slice(0, 24)} delay={0.08 * (index + 1)}>
          ...
          <FadeIn delay={0.15}>
            <aside className="border border-[var(--line)] p-6 md:p-8">
              ...
              <p className="mt-4 font-display text-2xl font-bold">{site.role}</p>
              <p className="mt-2 text-sm text-[var(--fg-muted)]">{site.location}</p>
              <a
        ...
        <div className="mt-24 border-t border-[var(--line)] pt-16">
          ...
          <ul className="mt-10">
            {experience.map((item, index) => (
              <li key={item.company} className="project-row">
